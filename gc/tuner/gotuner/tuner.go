...
func getGOGC(previousGOGC int, memoryLimitInPercent, memPercent float64) int {
	...
	logger.Debug(fmt.Sprintf("gctuner info: previousGOGC: %d, memoryLimitInPercent: %.04f, memPercent: %.04f, newgogc: %.04f", previousGOGC, memoryLimitInPercent, memPercent, newgogc))
...
func NewTuner2(useCgroup bool, percent float64, options ...OptFunc) *finalizer {
	errs := []error{}
