...
func generateExcel(data []User) (bool, error) {
	...
	for i := 1; i <= sheetNums; i++ {
		...
		go createSheet(&wg, sheet, titleRowInfo, data[(i - 1) * sheetRows:i * sheetRows], i)
	}

	wg.Wait()
	log.Printf("before save")
	file.Save(fmt.Sprintf("xlsx/io/user.xlsx"))
	log.Printf("after save")
	return true, nil
}

func createSheet(wg *sync.WaitGroup, sheet *xlsx.Sheet, titleRowInfo []string, data []User, i int)  {
